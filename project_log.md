...
### 2026-05-14 작업 내역
**프롬프트**: "Phase1에 추가로 mlp_model.model을 시각화하는 파일을 추가로 만들거야. 어떤 과정으로 만들어야 하는지 Phase1_Visualization.txt로 써줘"
**실행 내용**:
- `models/Phase1_Visualization.txt` 파일을 생성하여 저장된 모델 파일의 구조, 가중치, 예측 결과를 시각화하는 파이썬 스크립트 작성 가이드를 정리함.

### 2026-05-14 작업 내역 (2)
**프롬프트**: "pandas로 하는 걸로 다시 작성해주라"
**실행 내용**:
- `models/Phase1_Visualization.txt` 파일의 내용을 `pandas` DataFrame을 활용하여 모델의 예측 결과 분석, 오답 데이터 필터링, 그리고 내장 그래프 기능을 이용한 시각화 방법으로 전면 재작성함.
